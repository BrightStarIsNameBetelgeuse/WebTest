...
{ 
    public class GaussSolveStrategy : ISolveStrategy
    {
        ...
        public GaussSolveStrategy(double[,] a_matrix, double[] b_vector, double eps)
        {
        ...
        private double FindR(int row, int[] index)
        {
            ...
            if (max_abs < eps)
            {
                if (Math.Abs(b_vector[row]) > eps)
                    throw new GaussSolveNotFound("Система уравнений несовместна.");
                else
                    throw new GaussSolveNotFound("Система уравнений имеет множество решений.");
            }
